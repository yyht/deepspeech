...
output_list = []
import pandas as pd
df = pd.read_excel("/data/albert/asr/gaode_asr/全量涉性关键词20210521.xlsx", '行中语音关键词')

import re
CH_PUNCTUATION = u"['\\\\,\\!#$%&\'()*+-/:￥;<=>.?\\。n@[\\]^▽_`{|}~'－＂＃＄％＆＇，：；＠［＼］＾＿｀｛｜｝～｟｠｢｣、〃〜〝〞〟〰〾〿–—‘’‛“”„‟…‧﹏﹑﹔·！？｡]"


all_word_list = [re.sub(CH_PUNCTUATION, "", df.loc[index]['关键词']) for index in range(df.shape[0])]
asr_keyword_api = KeywordProcessor()
for word in all_word_list:
    asr_keyword_api.add_keyword(word, word)
    
import csv
import numpy as np
output_list = []
import pandas as pd
df = pd.read_excel("/data/albert/asr/gaode_asr/全量涉性关键词20210521.xlsx", '词包关键词')
...
all_word_list = [re.sub(CH_PUNCTUATION, "", df.loc[index]['关键词']) for index in range(df.shape[0])]
kfc_keyword_api = KeywordProcessor()
for word in all_word_list:
    kfc_keyword_api.add_keyword(word, word)

idst_path = '/data/albert/asr/gaode_asr/gaode_xingzhong_case23_idst.result'
cro_path = '/data/albert/asr/gaode_asr/gaode_xingzhong_case23_cro.result'

def word_match(data_path, keyword_api):
  ordered_dict = {}
  with open(data_path, "r") as frobj:
    for line in frobj:
      content = line.strip().split()
      order_id = content[0].split("_")[0]
      try:
        text = content[1]
        text = re.sub(CH_PUNCTUATION, "", text)
        result = keyword_api.extract_keywords(text)
      except:
        result = []
        text = ""
      if order_id in ordered_dict:
        ordered_dict[order_id].append(
            {
                "wave":content[0],
                "text":text,
                "keyword":result
...
idst_ordered_dict = word_match(idst_path, kfc_keyword_api)
cro_ordered_dict = word_match(cro_path, kfc_keyword_api)

cro_valid_count = 0
cro_sent_count = 0
for order_id in cro_ordered_dict:
  flag = False
  for item in cro_ordered_dict[order_id]:
    if len(item['keyword']):
      item['is_hitted_keyword'] = 1
      cro_sent_count += 1
      flag = True
  if flag:
    cro_valid_count += 1
    cro_ordered_dict[order_id]

idst_valid_count = 0
idst_sent_count = 0
for order_id in idst_ordered_dict:
  flag = False
  for item in idst_ordered_dict[order_id]:
    if len(item['keyword']):
      item['is_hitted_keyword'] = 1
      idst_sent_count += 1
      flag = True
...
idst_ordered_dict = word_match(idst_path, asr_keyword_api)
cro_ordered_dict = word_match(cro_path, asr_keyword_api)
